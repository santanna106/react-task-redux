...
function TaskItem({ task, onToggleComplete, onEdit, onDelete }) {
	...
	return (
		<div className='flex items-center gap-3 py-2'>
			...
			>
				{task.completed && <span className='material-icons text-black text-sm'>check</span>}
			</button>

			<div className='flex-grow'>
				<div className={`${theme.textSecondary} text-xs mb-1`}>{task.date}</div>
				<div className={`${theme.textPrimary} ${task.completed ? 'line-through opacity-60' : ''}`}>
					{task.title}
				</div>
				{task.description && (
					<div
						className={`${theme.textMuted} text-sm mt-1 ${
			...
			<div className='flex gap-2'>
				<button
					...
					className={`material-icons ${theme.textMuted} hover:${theme.textPrimary.replace(
						'text-',
						'text-'
